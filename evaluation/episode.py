...
def setup_dynamic_distractor_info(
    ...
    if any(a is not None for a in step_addr_y) and unrelated_addr:
        ...
        log_message(
            "[WARN] Insufficient dynamic information (no related or unrelated objects), feature disabled.",
            log_file,
        )
        return None
...
def initialize_episode_state(
    ...
    if initial_state is not None:
        try:
            ...
            log_message(
                f"Applied init state from init files for {task_type}/{case_name}",
                log_file,
            )
    ...
    if (cfg.resume or cfg.dynamic_shift_description) and step_states:
        ...
        if (
            ...
            if goal:
                comp_before_shift, total_completed_before_shift, _ = env._check_success(
                    goal
                )
                log_message(
                    f"[Dynamic Shift] Initial completion state: {total_completed_before_shift} total, details: {comp_before_shift}",
                    log_file,
                )
                ...
                if hasattr(env, "_state_progress"):
                    ...
                    log_message(
                        f"[Dynamic Shift] Reset state progress to initial values",
                        log_file,
                    )
            ...
            if goal:
                comp_after_shift, total_completed_after_shift, _ = env._check_success(
                    goal
                )
                log_message(
                    f"[Dynamic Shift] Post-resume completion state: {total_completed_after_shift} total, details: {comp_after_shift}",
                    log_file,
                )
            ...
            log_message(
                f"[Dynamic Shift] Jumped to Step {episode_stats['init_state_idx']}, logically completed {resume_completed_count} prior subtasks: {completed_subtasks}",
                log_file,
            )
            log_message(
                f"[Dynamic Shift] Excluding {resume_completed_count} subtasks from total goals due to forced completion",
                log_file,
            )
...
def handle_dynamic_movement(
    ...
    obs = env._get_observations()
    log_message(
        f"[Dynamic] Moved {target_name} object at step {step_idx}, Δy={offset:+.2f}",
        log_file,
    )
...
def handle_segment_transition(
    ...
    if do_resume_now and step_states is not None:
        ...
        if resume_handler and step_idx < len(naming_step_desc):
            ...
            episode_stats["total_resume_completed"] += resume_completed_count
            log_message(
                f"[Resume] Reverting to Step {step_idx}, auto-completed {resume_completed_count} prior subtasks: {completed_subtasks}",
                log_file,
            )
        else:
            log_message(
                f"[Resume] Reverting to the initial state of Step {step_idx}", log_file
            )
...
def execute_policy_step(
    cfg: GenerateConfig, model: RoboCerebraModel, observation: Dict, desc: str
) -> np.ndarray:
    """Execute a single policy inference step and return the action."""
    return model.predict_action(observation, desc)
...
def update_completion_tracking(
    ...
    if diff > 0:
        ...
        if episode_stats["skip_increment"]:
            ...
            log_message(
                f"[Step {step_no}] (Skip) Directly completed {diff} subtasks by resuming at the first frame",
                log_file,
            )
        else:
...
def finalize_episode(
    ...
    effective_total_goals = (
        episode_stats["total_goals"] - episode_stats["total_dynamic_shift_excluded"]
    )
    log_message(
        f"[Statistics] Original total goals: {episode_stats['total_goals']}, Dynamic shift excluded: {episode_stats['total_dynamic_shift_excluded']}, Effective total goals: {effective_total_goals}",
        log_file,
    )
